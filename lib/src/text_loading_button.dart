...
///
/// 由于逻辑冲突原因，在自动管理按钮中不能使用[TextLoadingButton.loadingPressable]参数，
/// 如有需要请自行使用[TextLoadingButton]。
///
/// [switchDuration]为状态变化时导致的控件大小变化而执行的过渡动画时间，内部使用[AnimatedSize]实现，默认为[kThemeAnimationDuration]。
///
/// [style]请参考使用[TextButton.styleFrom]生成
